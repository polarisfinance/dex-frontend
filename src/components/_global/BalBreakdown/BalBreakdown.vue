...
<template>
  <div>
    ...
    <div v-if="!hideItems" :class="['flex flex-col', offsetClass]">
      <div v-for="(item, i) in items" :key="i" :class="['flex items-center', { [spacingClass]: i !== 0 }]">
...
<style scoped>
.horiz-bar {
  @apply mr-2 h-px w-3 bg-gray-200 dark:bg-gray-700;
}

.init-vert-bar {
  @apply -mr-px w-px bg-gray-200 dark:bg-gray-700;
}
