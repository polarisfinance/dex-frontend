 ...
 * COMPUTED
 */
const fromPoolLoading = computed(() => fromPoolQuery.isLoading.value || fromPoolQuery.isIdle.value);

const toPoolLoading = computed(() => toPoolQuery.isLoading.value || toPoolQuery.isIdle.value);

const isLoadingPools = computed(() => toPoolLoading.value || fromPoolLoading.value);
...
const fromPoolTokenInfo = computed(() => (fromPool.value != null ? getToken(fromPool.value.address) : null));

const toPoolTokenInfo = computed(() => (toPool.value != null ? getToken(toPool.value.address) : null));
</script>
...
<template>
  <Col3Layout offsetGutters>
    ...
    <BalLoadingBlock v-if="isLoadingPools || fromPoolTokenInfo == null || toPoolTokenInfo == null || batchRelayerApprovalLoading" class="h-96" />
    <PoolsInfo v-else :fromPool="fromPool" :toPool="toPool" :fromPoolTokenInfo="fromPoolTokenInfo" :toPoolTokenInfo="toPoolTokenInfo" :poolMigrationInfo="poolMigrationInfo" />
