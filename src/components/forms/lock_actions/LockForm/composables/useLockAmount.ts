...
export default function useLockAmount(veBalLockInfo?: Ref<VeBalLockInfo> | Ref<undefined>) {
  /**
   * COMPOSABLES
   */
  ...
  const isIncreasedLockAmount = computed(() => veBalLockInfo?.value?.hasExistingLock && isValidLockAmount.value);
