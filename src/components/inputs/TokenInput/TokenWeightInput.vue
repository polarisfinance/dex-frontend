...
<template>
  ...
  >
    <template #prepend>
      <TokenSelectInput v-model="_address" :fixed="fixedToken" class="mr-2" :excludedTokens="excludedTokens" @update:model-value="emit('update:address', $event)" />
    </template>
    <template #append>
      <div class="flex h-full flex-row items-center justify-center">
        <span class="text-[24px] font-medium leading-[31px] text-pink-third">%</span>
        <button
          :class="[
            'text-secondary  ml-[16px]  flex items-center justify-center rounded-full bg-[#2E2433] bg-none px-[10px] py-[8px]',
            {
        ...
        <button :class="['text-secondary ml-[8px] flex items-center justify-center rounded-full bg-[#2E2433] bg-none px-[9px] py-[8px]']" @click="emit('delete')">
