...
const stats = computed(() => {
  ...
  let outStats = [
    ...
    },
    {
      id: 'apr',
      label: 'APR',
      value: Number(props.poolApr?.total.unstaked || '0') * 100 > APR_THRESHOLD ? '-' : aprLabel.value,
...
<template>
  <div class="flex w-full" :class="{ 'flex-wrap': isMobile, 'mb-[24px]': isDesktop }">
    <template v-for="stat in stats" :key="stat.id">
      ...
      <div v-else class="card-container flex flex-1" :class="{ 'justify-center': isDesktop, 'mb-10 justify-start': isMobile }">
        <div>
        ...
        <div class="ml-[16px]">
          ...
          <div class="funds flex items-center truncate text-xl font-medium" v-if="stat.label != 'APR'">{{ stat.value }} <span v-if="stat.label == 'APR'">%</span></div>
          <div class="funds flex items-center truncate text-xl font-medium" v-else>{{ aprString }} <span v-if="stat.label == 'APR'">%</span></div>
        </div>
      </div>
    </template>
    <div class="pool-invest w-full text-center" v-if="isMobile">
      Invest in the pool and earn on swap fees!
      <router-link class="invest-btn flex w-full items-center" :to="'/pool/' + pool?.id + '/invest'">
      ...
      <div class="card-container flex flex-1" :class="{ 'my-8': isMobile }" v-if="account && Number(stakedBalance) > 0">
        <div class="card-container flex flex-1">
          ...
          <div class="ml-[16px]">
            ...
            <div class="funds claim flex items-center truncate text-xl font-medium">
              {{ rewardFiat }}
            </div>
          </div>
        </div>
  ...
  <div class="buttons-panel flex text-center" :class="{ 'flex-wrap': isMobile }">
    ...
    <div class="my-panel flex flex-1 gap-4" v-if="account && Number(stakedBalance) > 0" :class="{ 'l-border pl-[24px]': isDesktop, 'flex-col': isMobile }">
      <div class="pool-invest flex-1">
        ...
        <button class="invest-btn block flex w-full items-center" @click="claimXpolar(pool.address)">
      ...
      <div class="pool-invest flex-1">
        ...
        <router-link class="goto-btn flex w-full items-center" :to="'#dashboard'">
