          ...
          <div class="steps flex justify-between">
            <template v-for="step in steps">
              <button @click="setActiveStep(step.step)" class="self-start">
                <TickIcon v-if="step.step < activeStep || activeStep == steps.length" class="" :class="{ 'mx-auto block': isMobile, 'mr-1 inline': isDesktop }" />
                <QuestionIcon v-else class="" :class="{ 'mx-auto block': isMobile, 'mr-1 inline': isDesktop }" />
        ...
        <div class="flex-1 justify-center">
          <div class="header flex">
            ...
            <div class="flex-1 text-right">
              <router-link :to="{ name: 'pool', params: { id: id } }"> Exit <CloseIcon class="ml-[12px] inline" /> </router-link>
            </div>
          </div>
            ...
            <div class="nested-card mt-[16px]">
              ...
              <template v-if="activeStep == 4">
                <div class="finished text-center">
                  ...
                  <button class="exit mt-[20px] inline-block" @click="approvePool">Approve staking</button>
                </div>
              </template>
              ...
              <template v-if="activeStep == 6">
                <div class="finished text-center">
                  <svg class="mx-auto mt-[70px] mb-5" width="64" height="64" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path
                  ...
                  <router-link class="exit mt-[20px] inline-block" :to="{ name: 'pool', params: { id: id } }"> Return to pool </router-link>
                </div>
              </template>
            </div>
          </div>
