  ...
  <BalModal :show="isVisible" noPad @close="$emit('close')">
    <div class="p-[12px]">
      <div class="header px-[12px]">
        <div v-if="purchaseBol" class="title text-white">Purchase {{ name }}</div>
      ...
      <div class="grid justify-items-start pt-[24px]">
        ...
        <button class="button-style mt-[12px] h-[44px] w-full rounded-[16px] text-white" @click="purchaseBol ? purchase(inputValue) : redeem(inputValue)">Confirm</button>
      </div>
    </div>
  </BalModal>
</template>
