...
import { computed, defineComponent, h, InjectionKey, provide, reactive, Ref, ref } from 'vue';
...
import useUserStakingData, { UserStakingDataResponse } from './userUserStakingData';
...
export async function getGaugeAddress(poolAddress: string, provider: JsonRpcProvider): Promise<string> {
  const gaugeInterface = new Interface(GaugeFactoryABI);
  const contract = new Contract(configService.network.addresses.gaugeFactory, gaugeInterface, provider);
 ...
 * SETUP
 */
export const StakingProviderSymbol: InjectionKey<StakingProvider> = Symbol(symbolKeys.Providers.App);
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const isPoolAddressRegistered = computed(() => !!poolAddress.value && poolAddress.value != '');

    // this query is responsible for checking if the given pool
    // is eligible for staking rewards or not
    const { data: poolEligibilityResponse, isLoading: isLoadingPoolEligibility } = useGraphQuery<{ liquidityGauges: TLiquidityGauge[] }>(
      subgraphs.gauge,
    ...
    const isPoolEligibleForStaking = computed(() => (poolEligibilityResponse.value?.liquidityGauges || [])[0]?.id !== undefined);
    ...
    async function stakeBPT() {
      if (!poolAddress.value) {
        throw new Error(`Attempted to call stake, however useStaking was initialised without a pool address.`);
      }
      const gaugeAddress = await getGaugeAddress(poolAddress.value, getProvider());
      const gauge = new LiquidityGauge(gaugeAddress);
      const tx = await gauge.stake(parseUnits(balanceFor(getAddress(poolAddress.value)), 18));
      return tx;
    }

    async function unstakeBPT() {
      if (!poolAddress.value) {
        throw new Error(`Attempted to call unstake, however useStaking was initialised without a pool address.`);
      }
      const gaugeAddress = await getGaugeAddress(getAddress(poolAddress.value), getProvider());
      const gauge = new LiquidityGauge(gaugeAddress);
      const tx = await gauge.unstake(parseUnits(stakedSharesForProvidedPool.value || '0', 18));
      return tx;
    }
