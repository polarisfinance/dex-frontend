 ...
 */
export default function useTokenPricesQuery(addresses: Ref<string[]> = ref([]), pricesToInject: Ref<TokenPrices> = ref({}), options: UseQueryOptions<QueryResponse> = {}) {
  const { networkId } = useNetwork();
  const queryKey = reactive(QUERY_KEYS.Tokens.Prices(networkId, addresses, pricesToInject));
  ...
  const queryFn = async () => {
    ...
    for (const page of pages) {
      ...
      const pageAddresses = addresses.value.slice(PER_PAGE * page, PER_PAGE * (page + 1));
