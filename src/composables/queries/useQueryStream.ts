...
import { flatten, get, isArray, last, mapValues, merge, nth, tail } from 'lodash';
import { QueryKey } from 'react-query';
import { computed, reactive, Ref, ref, watch } from 'vue';
import { useQueries } from 'vue-query';

export function queryTemplatesEmitter(queryTemplates: Promise<any>[] | Promise<any>) {
...
export default function useQueryStreams(id: string, queryTemplates: queryTemplates) {
  const currentPage = ref(1);
  const initQuery = computed(() => Object.values(queryTemplates).find(query => query.init === true));
  if (!initQuery.value) {
    throw new Error(`A set of parallel queries needs an initial query. Please mark one of the passed queries as [init].`);
  }
  const initialQueryHash = computed(() => JSON.stringify(initQuery.value?.dependencies));

  const currentPageData = ref((result.value[initialQueryHash.value] || [])[result.value[initialQueryHash.value]?.length - 1]);
  ...
  const successStates = ref({
    [initialQueryHash.value]: Object.fromEntries(Object.keys(queryTemplates).map(queryKey => [queryKey, false])),
  });

  // Same as success, internal data is keyed by the query hash first, and then the id
  // of the query. As query hashes may represent different filters etc
  const internalData = ref({
    [initialQueryHash.value]: Object.fromEntries(Object.keys(queryTemplates).map(queryKey => [queryKey, undefined])),
  });
  ...
  watch(initialQueryHash, () => {
    ...
    const independentQueries = Object.keys(queryTemplates).filter(queryKey => ['independent', 'paged_independent'].includes(queryTemplates[queryKey].type || ''));
    ...
    internalData.value[initialQueryHash.value] = Object.fromEntries(
      Object.keys(queryTemplates).map(queryKey => {
        ...
        if (independentQueries.includes(queryKey)) {
          const independentData = Object.values(internalData.value)[0][queryKey];
  ...
  Object.keys(queryTemplates).forEach(queryKey => {
    ...
    const otherQueryDependency = computed(() => {
      return (query.waitFor || []).map(queryId => {
        if (queryId.includes('.')) {
          return (internalData.value[initialQueryHash.value][queryId.split('.')[0]] || []).map(data => get(data, tail(queryId.split('.')).join('.')));
        }
    ...
    const template: any = {
      queryFn: async () => {
        ...
        if (!query.streamResponses) {
          const res = await query.queryFn(currentPageData, ref(internalData.value[initialQueryHash.value]), currentPage, successStates, initialQueryHash);
        ...
        const emitter = queryTemplatesEmitter(await query.queryFn(currentPageData, ref(internalData.value[initialQueryHash.value]), currentPage, successStates));
      ...
      enabled: computed(
        () => (query.enabled?.value ?? true) && (queryTemplates[queryKey].waitFor || []).every(otherQueryId => successStates.value[initialQueryHash.value][otherQueryId.split('.')[0]] === true)
      ),
      refetchOnWindowFocus: false,
      keepPreviousData: true,
    };
    template.onSuccess = response => {
      if (!query.streamResponses && !['independent', 'page_dependent'].includes(query.type || '')) {
        if (!result.value[initialQueryHash.value]) {
          result.value[initialQueryHash.value] = [];
        }
        if ((result.value[initialQueryHash.value] || []).length < currentPage.value) {
          ...
          result.value[initialQueryHash.value][currentPage.value - 1] = merge(result.value[initialQueryHash.value][currentPage.value - 1], response);
          currentPageData.value = response;
        }
      }
  ...
  const dataStates = computed(() => {
    type DataState = {
      [key: keyof typeof queryTemplates]: 'idle' | 'error' | 'loading' | 'success';
    };
  ...
  const isComplete = computed(() => Object.values(last(Object.values(dataStates.value)) || []).every(state => state === 'success'));

  const _result = computed(() => flatten(result.value[initialQueryHash.value]));
  const currentDataStates = computed(() => last(Object.values(dataStates.value)) || {});
  const isLoadingMore = computed(() => {
    if (Object.values(dataStates.value).length > 1) {
      if (Object.values(nth(Object.values(dataStates.value), -2) || {}).some(state => state === 'success')) {
        if (Object.values(last(Object.values(dataStates.value)) || {}).some(state => state === 'loading')) {
          return true;
        }
      }
    }
  ...
  function loadMore() {
    currentPage.value += 1;
    successStates.value[initialQueryHash.value] = mapValues(successStates.value[initialQueryHash.value], (state, key) => {
      if (['independent', 'paged_independent'].includes(queryTemplates[key].type || '')) {
        return true;
      }
      return false;
    });
