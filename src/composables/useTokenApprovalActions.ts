...
import useTokenApprovals, { ApprovalStateMap } from '@/composables/pools/useTokenApprovals';
...
export default function useTokenApprovalActions(tokenAddresses: string[], amounts: Ref<string[]>) {
  /**
   * COMPOSABLES
   */
  const { t } = useI18n();
  const { getToken } = useTokens();
  const { vaultApprovalStateMap, approveToken, getApprovalStateMapFor } = useTokenApprovals(tokenAddresses, amounts);
   ...
   * METHODS
   */
  async function getTokenApprovalActionsForSpender(spender: string, amount: string = MaxUint256.toString()) {
    const stateMap = await getApprovalStateMapFor(spender);
    return getTokenApprovalActions({ spender, amount, stateMap });
  }

  function getTokenApprovalActions(options: Partial<ApprovalActionOptions> = {}): TransactionActionInfo[] {
    const defaultOptions: ApprovalActionOptions = {
      spender: vaultAddress,
      amount: MaxUint256.toString(),
      stateMap: vaultApprovalStateMap.value,
    };
    const { spender, amount, stateMap } = Object.assign(defaultOptions, options);

    return Object.keys(stateMap).map(address => {
      const token = getToken(address);
      const state = stateMap[address];
      return {
        label: t(spender === appNetworkConfig.addresses.veBAL ? 'transactionSummary.approveForLocking' : 'transactionSummary.approveForInvesting', [token.symbol]),
