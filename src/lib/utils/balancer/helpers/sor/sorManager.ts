import { PoolFilter, SOR as SORV2, SubgraphPoolBase, SwapInfo, SwapOptions, SwapTypes } from '@balancer-labs/sdk';
...
export class SorManager {
  ...
  constructor(provider: Provider, gasPrice: BigNumber, maxPools: number, chainId: number, weth: string) {
  ...
  async setCostOutputToken(tokenAddr: string, tokenDecimals: number, manualCost: string | null = null): Promise<BigNumber> {
    tokenAddr = tokenAddr === NATIVE_ASSET_ADDRESS ? this.weth : tokenAddr;

    if (manualCost) {
      await this.sorV2.swapCostCalculator.setNativeAssetPriceInToken(tokenAddr, manualCost);
    }

    const cost = await this.sorV2.getCostOfSwapInToken(tokenAddr, tokenDecimals, this.gasPrice, BigNumber.from(SWAP_COST));
  ...
  async fetchPools(): Promise<void> {
    ...
    try {
      ...
      console.log(`[SorManager] V2 fetchPools issue: ${(err as Error).message}`);
      this.fetchStatus.v2finishedFetch = true;
  ...
  async getBestSwap(tokenIn: string, tokenOut: string, tokenInDecimals: number, tokenOutDecimals: number, swapType: SwapTypes, amountScaled: OldBigNumber): Promise<SorReturn> {
    const v2TokenIn = tokenIn === NATIVE_ASSET_ADDRESS ? AddressZero : tokenIn;
    const v2TokenOut = tokenOut === NATIVE_ASSET_ADDRESS ? AddressZero : tokenOut;
    ...
    const swapInfoV2: SwapInfo = await this.sorV2.getSwaps(v2TokenIn.toLowerCase(), v2TokenOut.toLowerCase(), swapType, BigNumber.from(amountScaled.toString()), swapOptions);

    // Both are scaled amounts
    console.log(`[SorManager] ${swapInfoV2.returnAmount.toString()}: V2 return amount`);
    console.log(`[SorManager] ${swapInfoV2.returnAmountConsideringFees.toString()}: V2 return amount with fees`);

    return {
      tokenIn,
      tokenOut,
      returnDecimals: swapType === SwapTypes.SwapExactIn ? tokenOutDecimals : tokenInDecimals,
  ...
  hasPoolData(): boolean {
    if (this.fetchStatus.v2finishedFetch) {
      ...
      if (this.fetchStatus.v2success === false) {
        console.log(`[SorManager] Error Fetching V2 Pools - No Liquidity Sources.`);
        return false;
      }
