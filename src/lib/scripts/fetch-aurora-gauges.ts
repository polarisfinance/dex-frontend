/**
 * Fetches Aurora gauge addresses directly from GaugeFactory on-chain.
 *
 * Run with: npm run fetch:aurora-gauges
 */
...
const CHAIN_ID = 1313161554; // Aurora mainnet

// Aurora contract addresses
const GAUGE_FACTORY = '0x80C296469a1ffFfB6547A8570927864708b86303';

// All VotingGaugePools from pools.ts
const VOTING_GAUGE_POOLS = [
...
// GaugeFactory ABI (just the function we need)
const GAUGE_FACTORY_ABI = [
  'function getPoolGauge(address pool) view returns (address)',
];

// Extract pool address from pool ID (first 20 bytes)
function getPoolAddress(poolId: string): string {
  return getAddress(poolId.slice(0, 42));
}

async function main() {
  console.log('Fetching Aurora gauge addresses from chain...\n');

  const provider = new StaticJsonRpcProvider(RPC_URL, { chainId: CHAIN_ID, name: 'aurora' });
  const gaugeFactory = new Contract(GAUGE_FACTORY, GAUGE_FACTORY_ABI, provider);

  const results: Record<string, { poolAddress: string; gaugeAddress: string }> = {};
  const errors: string[] = [];

  for (const poolId of VOTING_GAUGE_POOLS) {
    const poolAddress = getPoolAddress(poolId);

    try {
      const gaugeAddress = await gaugeFactory.getPoolGauge(poolAddress);

      if (gaugeAddress && gaugeAddress !== AddressZero) {
        results[poolId] = {
          poolAddress,
          gaugeAddress,
        };
        console.log(`✓ ${poolId.slice(0, 10)}... -> ${gaugeAddress}`);
      } else {
        errors.push(`No gauge for pool ${poolId.slice(0, 10)}...`);
        console.log(`✗ ${poolId.slice(0, 10)}... -> No gauge found`);
      }
    } catch (error) {
  ...
  console.log('\n--- RESULTS ---\n');
  console.log('// Add this to src/lib/config/aurora/gauges.ts\n');
  console.log('export const AURORA_POOL_GAUGES: Record<string, string> = {');
  for (const [poolId, data] of Object.entries(results)) {
    console.log(`  // Pool: ${data.poolAddress}`);
    console.log(`  '${poolId}': '${data.gaugeAddress}',`);
  }
  console.log('};');
  ...
  console.log(`\nTotal: ${Object.keys(results).length} gauges found out of ${VOTING_GAUGE_POOLS.length} pools`);
}
