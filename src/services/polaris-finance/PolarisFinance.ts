...
export class PolarisFinance {
  ...
  constructor(cfg: Configuration) {
    ...
    this.BINARIS_METAMASK = this.BINARIS;
    this.BBOND_METAMASK = this.BBOND;
  ...
  async watchAssetInMetamask(assetName: string): Promise<boolean> {
    ...
    if (ethereum && ethereum.networkVersion === config.chainId.toString()) {
      ...
      } else if (assetName === 'BINARIS') {
        asset = this.BINARIS;
        assetUrl = 'https://polarisfinance.io/logos/binaris-token.svg';
      } else if (assetName === 'BBOND') {
        asset = this.BBOND;
        assetUrl = 'https://polarisfinance.io/logos/bbond-token.svg';
      }
  ...
  async earnedFromBank(
    ...
    try {
      ...
      if (earnTokenName === 'BINARIS') {
        return await pool.pendingBinaris(poolId, account);
      } else {
  ...
  async getTokenPerSecond(
    ...
    if (earnTokenName === 'BINARIS') {
      const rewardPerSecond = await poolContract.binarisPerSecond();
      if (depositTokenName === 'BNB') {
        return rewardPerSecond.mul(50000).div(100000);
      } else if (depositTokenName === 'SPOLAR') {
        return rewardPerSecond.mul(20000).div(100000);
      } else if (depositTokenName === 'POLAR') {
        return rewardPerSecond.mul(1000).div(100000);
      } else if (depositTokenName === 'ETHERNAL') {
        return rewardPerSecond.mul(1000).div(100000);
      } else if (depositTokenName === 'POLAR-NEAR-LP') {
        return rewardPerSecond.mul(7000).div(100000);
      } else if (depositTokenName === 'SPOLAR-NEAR-LP') {
        ...
        return rewardPerSecond.mul(2000).div(100000);
      } else {
        return rewardPerSecond.mul(500).div(100000);
      }
    }
    const [
      rewardPerSecond,
    ...
    } else if (depositTokenName.startsWith('POLAR-USP')) {
      return rewardPerSecond.mul(PolarUsp.allocPoint).div(41000);
    } else if (depositTokenName.startsWith('ETHERNAL-USP')) {
      return rewardPerSecond.mul(EthernalUsp.allocPoint).div(41000);
    } else if (depositTokenName.startsWith('ORBITAL-USP')) {
      return rewardPerSecond.mul(OrbitalUsp.allocPoint).div(41000);
