...
export function getInjectedProvider() {
  const ethereum: any = (window as any).sender?.ethereum;
  let provider = ethereum || (window as any).web3?.currentProvider;

  // if multiple providers are injected and one of them is Sender, prefer Sender
  if (ethereum?.providers?.length) {
    ethereum.providers.forEach(p => {
      if (p.isSender) provider = p;
    });
  }
  console.log(provider === ethereum);
  return provider;
}
...
export class SenderConnector extends Connector {
  id = ConnectorId.InjectedSender;
  async connect() {
    ...
    if (provider) {
      ...
      try {
        ...
        if ((err as WalletError).code === 4001) {
          ...
          console.log('Please connect to Sender Wallet.');
        } else {
          console.error(err);
        }
      }
      ...
      console.error(
        'Tried to connect to Sender Wallet but it was not detected. Please install Sender Wallet.'
      );
    }
    return {
      // TODO type this
...
export const SenderConnectorType = typeof SenderConnector;
