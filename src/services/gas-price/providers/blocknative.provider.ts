...
export default class BlocknativeProvider {
  public async getLatest(confidence: BlocknativeGasPriceConfidence | 'best' = 'best'): Promise<GasPrice | null> {
    try {
      const response = await axios.get<BlocknativeGasPlatformResponse>('https://api.blocknative.com/gasprices/blockprices', {
        headers: {
      ...
      if (confidence === 'best') {
        const gasPrice70 = estimatedPrices.find(estimatedPrice => estimatedPrice.confidence === 70);
        const gasPrice90 = estimatedPrices.find(estimatedPrice => estimatedPrice.confidence === 90);

        if (gasPrice70 != null && gasPrice90 != null) {
          gasPrice = gasPrice90.price > 1.25 * gasPrice70.price ? gasPrice70 : gasPrice90;
        }
      } else {
        gasPrice = estimatedPrices.find(estimatedPrice => estimatedPrice.confidence === confidence);
      }
      ...
      if (gasPrice != null) {
        return {
          ...
          maxPriorityFeePerGas: Math.round(gasPrice.maxPriorityFeePerGas * GWEI_UNIT),
        };
      }
    } catch (e) {
