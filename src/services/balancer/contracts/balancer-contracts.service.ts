import { InvestmentPool__factory, StablePool__factory, WeightedPool__factory } from '@balancer-labs/typechain';
...
export default class BalancerContractsService {
  ...
  constructor(readonly configService = _configService, readonly rpcProviderService = _rpcProviderService, readonly sdk = balancer) {
  ...
  public get allPoolABIs() {
    return Object.values(
      Object.fromEntries(
        [...WeightedPool__factory.abi, ...StablePool__factory.abi, ...InvestmentPool__factory.abi, ...StablePhantomPool, ...LinearPoolAbi, ...StaticATokenLMAbi, ...ERC20_ABI, ...IERC4626].map(row => [
          row.name,
          row,
        ])
      )
