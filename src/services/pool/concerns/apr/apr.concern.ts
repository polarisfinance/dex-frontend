...
export class AprConcern {
  constructor(public pool: Pool, private readonly lido = lidoService, private readonly aave = aaveService, private readonly VeBalAprCalcClass = VeBalAprCalc) {}

  public async calc(poolSnapshot: Pool | undefined, prices: TokenPrices, currency: FiatCurrency, protocolFeePercentage: number, stakingBalApr: AprRange, stakingRewardApr = '0'): Promise<PoolAPRs> {
    const swapFeeAPR = this.calcSwapFeeAPR(poolSnapshot, protocolFeePercentage);

    const yieldAPR = await this.calcYieldAPR(prices, currency, protocolFeePercentage);

    const veBalAPR = await this.calcVeBalAPR(prices);

    const unstakedTotalAPR = bnSum([swapFeeAPR, yieldAPR.total]).toString();

    const aprGivenBoost = (boost = '1') => this.calcAprGivenBoost(unstakedTotalAPR, stakingBalApr, stakingRewardApr, boost);

    const stakedAprRange = this.calcStakedAprRange(unstakedTotalAPR, stakingBalApr, stakingRewardApr);
   ...
   */
  private calcAprGivenBoost(unstakedTotalAPR: string, stakingBalApr: AprRange, stakingRewardApr = '0', boost = '1'): string {
   ...
   */
  private calcStakedAprRange(unstakedTotalAPR: string, stakingBalApr: AprRange, stakingRewardApr = '0'): AprRange {
    const stakedBaseAPR = bnum(unstakedTotalAPR).plus(stakingRewardApr);
   ...
   */
  private async calcYieldAPR(prices: TokenPrices, currency: FiatCurrency, protocolFeePercentage: number): Promise<{ total: string; breakdown: Record<string, string> }> {
    let total = '0';
    let breakdown = {};
    ...
    if (includesWstEth(this.pool.tokensList)) {
      ...
      const aaveAPR = await this.aave.calcWeightedSupplyAPRFor(this.pool, prices, currency);
      ...
      const usdPlusPools = {
        '0xb973ca96a3f0d61045f53255e319aedb6ed4924000000000000000000000042f': '0x1aAFc31091d93C3Ff003Cff5D2d8f7bA2e728425',
        '0xf48f01dcb2cbb3ee1f6aab0e742c2d3941039d56000000000000000000000445': '0x6933ec1CA55C06a894107860c92aCdFd2Dd8512f',
      };
      if (Object.keys(usdPlusPools).includes(this.pool.id)) {
        ...
        if (linearPool) {
          ...
          const weightedAPR = await calcUSDPlusWeightedAPR(this.pool, linearPool, linearPoolAddress, prices, currency);
  ...
  private async calcVeBalAPR(prices: TokenPrices): Promise<string> {
    ...
    return await veBalApr.calc(this.pool.totalLiquidity, this.pool.totalShares, prices);
  }
}
