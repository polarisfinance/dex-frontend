...
import { LinearPool, OnchainPoolData, Pool, PoolAPRs, PoolToken, RawOnchainPoolData } from '@/services/pool/types';
...
export default class PoolService {
  constructor(public pool: Pool, public liquidity = LiquidityConcern, public apr = AprConcern) {
    this.format();
  }
   ...
   */
  public setTotalLiquidity(prices: TokenPrices, currency: FiatCurrency, tokenMeta: TokenInfoMap = {}): string {
    const liquidityConcern = new this.liquidity(this.pool);
    const totalLiquidity = liquidityConcern.calcTotal(prices, currency, tokenMeta);
  ...
  public async setAPR(
    ...
    const apr = await aprConcern.calc(poolSnapshot, prices, currency, protocolFeePercentage, stakingBalApr, stakingRewardApr);
  ...
  public async setLinearPools(): Promise<Record<string, PoolToken> | null> {
    ...
    linearPools.forEach(linearPool => {
      ...
      const index = this.pool.tokensList.indexOf(linearPool.address.toLowerCase());

      this.pool.mainTokens[index] = linearPool.tokensList[linearPool.mainIndex];
      this.pool.wrappedTokens[index] = linearPool.tokensList[linearPool.wrappedIndex];
  ...
  removePreMintedBPT(): string[] {
    return (this.pool.tokensList = this.pool.tokensList.filter(address => !isSameAddress(address, this.pool.address)));
  }

  formatPoolTokens(): PoolToken[] {
    if (isStable(this.pool.poolType)) return this.pool.tokens;

    return (this.pool.tokens = this.pool.tokens.sort((a, b) => parseFloat(b.weight) - parseFloat(a.weight)));
  }

  public setFeesSnapshot(poolSnapshot: Pool | undefined): string {
    if (!poolSnapshot) return '0';

    const feesSnapshot = bnum(this.pool.totalSwapFee).minus(poolSnapshot.totalSwapFee).toString();
  ...
  public setVolumeSnapshot(poolSnapshot: Pool | undefined): string {
    ...
    const volumeSnapshot = bnum(this.pool.totalSwapVolume).minus(poolSnapshot.totalSwapVolume).toString();

    return (this.pool.volumeSnapshot = volumeSnapshot);
  }

  public setOnchainData(rawOnchainData: RawOnchainPoolData, tokenMeta: TokenInfoMap): OnchainPoolData | undefined {
    const onchainData = new OnchainDataFormater(this.pool, rawOnchainData, tokenMeta);
    return (this.pool.onchain = onchainData.format());
  }

  public setUnwrappedTokens(): string[] {
    const unwrappedTokens = Object.entries(this.pool?.onchain?.linearPools || {}).map(([, linearPool]) => linearPool.unwrappedTokenAddress);
    return (this.pool.unwrappedTokens = unwrappedTokens);
  }

  public get isNew(): boolean {
    return differenceInWeeks(Date.now(), this.pool.createTime * oneSecondInMs) < 1;
  }
}
