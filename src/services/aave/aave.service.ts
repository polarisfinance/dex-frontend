...
import AaveSubgraphService, { aaveSubgraphService } from './subgraph/aave-subgraph.service';
...
export default class AaveService {
  ...
  public async calcWeightedSupplyAPRFor(pool: Pool, prices: TokenPrices, currency: FiatCurrency) {
    const { mainTokens, wrappedTokens, linearPoolTokensMap } = pool;

    const wrappedTokenBalances = wrappedTokens?.map(token => linearPoolTokensMap?.[token].balance || '');
    const hasAllWrappedTokenBalances = wrappedTokenBalances && wrappedTokenBalances.every(balance => !!balance);
    ...
    wrappedTokens?.forEach((address, i) => {
      ...
      multicaller.call(`linearPoolTotalSupplies[${i}]`, pool.tokensList[i], 'getVirtualSupply');
    });

    const { unwrappedAave, unwrappedERC4626, linearPoolTotalSupplies } = await multicaller.execute();

    const mappedUnwrappedTokens = wrappedTokens?.map((address, i) => {
      return (unwrappedAave[i] || unwrappedERC4626[i] || this.addressMapIn(address)).toLowerCase();
    });

    if (!mainTokens || !wrappedTokens || !mappedUnwrappedTokens || !hasAllWrappedTokenBalances) return { total: '0', breakdown: {} };

    let total = bnum(0);
    const breakdown: Record<string, string> = {};

    const mappedMainTokens = mainTokens?.map(address => this.addressMapIn(address));
    ...
    reserves.forEach(reserve => {
      ...
      if (supplyAPR.gt(0)) {
        const tokenIndex = mappedMainTokens.findIndex(token => isSameAddress(token, reserve.underlyingAsset));
        ...
        const linearPoolTotalSupply = formatUnits(linearPoolTotalSupplies[tokenIndex], 18);
        ...
        if (mainTokenPrice && linearPoolTotalSupply) {
          ...
          const linearPoolShare = bnum(linearPoolBalance).div(linearPoolTotalSupply);
